...
//	private JPanel SeparatorCharacterPanel;
	...
	private void initGui() {
		...
		inputPanel.add(inputBrowseTextField,
				GuiUtil.setConstraints(0, 0, 1.0, 1.0, GridBagConstraints.HORIZONTAL, BT, BT * 6, BT, BT));
		inputBrowseTextField.setColumns(10);
		inputPanel.add(inputBrowseButton,
				GuiUtil.setConstraints(3, 0, 0.0, 0.0, GridBagConstraints.NONE, BT, BT, BT, BT));
...
//		containerPanel.add(Box.createRigidArea(new Dimension(0, BT * 6)));
		...
		zOffsetPanel.add(zOffsetInputField,
				GuiUtil.setConstraints(0, 0, 1.0, 1.0, GridBagConstraints.HORIZONTAL, BT, BT * 6, BT, BT));
		...
		zScaleFactorPanel.add(zScaleFactorInputField,
				GuiUtil.setConstraints(0, 0, 1.0, 1.0, GridBagConstraints.HORIZONTAL, BT, BT * 6, BT, BT));
		...
		tileSizePanel.add(tileSizeInputField,
				GuiUtil.setConstraints(0, 0, 1.0, 1.0, GridBagConstraints.HORIZONTAL, BT, BT * 6, BT, BT));
		...
		maxNumOfPointsPerTilePanel.add(maxNumOfPointsPerTileInputField,
				GuiUtil.setConstraints(0, 0, 1.0, 1.0, GridBagConstraints.HORIZONTAL, BT, BT * 6, BT, BT));
		...
		outputPanel.add(outputBrowserTextField,
				GuiUtil.setConstraints(0, 0, 1.0, 1.0, GridBagConstraints.HORIZONTAL, BT, BT * 6, BT, BT));
...
//		runButton.setText("Export");
		...
		inputPanel.setBorder(BorderFactory.createTitledBorder("输入路径 "));
		inputBrowseButton.setText("浏览...");
		sridPanel.setBorder(BorderFactory.createTitledBorder("空间参考SRID"));
//		SeparatorCharacterPanel.setBorder(BorderFactory.createTitledBorder("分隔符（默认空格"));
		colorBitSizePanel.setBorder(BorderFactory.createTitledBorder("颜色位大小8或16"));
		zScaleFactorPanel.setBorder(BorderFactory.createTitledBorder("高度调整尺度因子 (0.0 - 1.0) (默认1)"));
		zOffsetPanel.setBorder(BorderFactory.createTitledBorder("高度调整z偏移 (默认0)"));
		tileSizePanel.setBorder(BorderFactory.createTitledBorder("每个瓦片的边长(默认100)"));
		maxNumOfPointsPerTilePanel.setBorder(BorderFactory.createTitledBorder("每个瓦片的最大点数（默认10000个点）"));
		outputPanel.setBorder(BorderFactory.createTitledBorder("输出路径"));
		outputBrowseButton.setText("选择...");
	...
	private void addListeners() {
		...
		inputBrowseButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				FileNameExtensionFilter filter = new FileNameExtensionFilter("LAS Files", "LAS");
	...
	private void doProcess() {
		...
		if (zScaleFactorInputField.getText().trim() != "") {
			try {
				...
				if (zScaleFactor > 1 || zScaleFactor < 0) {
					errorMessage("Incorrect parameter value",
							"Invalid Z scale factor value, it must be a value in the range of (0.0 - 1.0)");
					return;
				}
		...
		// 自动生成目录，避免选择根目录是错误地删除数据
		// String fn = FileUtils.getFileName(inputPath);
		outputPath = outputPath + "\\" + UUID.randomUUID().toString();
...
//		final PntcGenerator generator = new PntcGenerator(config, dbManagerFactory);		
		...
		dlls.add("./plugins/JniUtils");
		dlls.add("./plugins/liblas");
		dlls.add("./plugins/liblasJNI");
		
		generator.initDLL(dlls);
	...
	private void initDefaultSettings() {
		...
		sridInputField.setText("32650");
		zScaleFactorInputField.setText("1");
		SeparatorCharacterInputField.setText(" ");
		colorBitSizeInputField.setText("8");
		maxNumOfPointsPerTileInputField.setText("10000");
		inputBrowseTextField.setText("");
		outputBrowserTextField.setText("");
	}
